# Contributing to codex-brave-web-search
...
3. Clone and build:
   ```sh
   git clone https://github.com/lammesen/codex-brave-web-search.git
   cd codex-brave-web-search
   cargo build
   ```
...
2. Make your changes, ensuring:
   - All tests pass (`cargo test --all-features --locked`)
   - Code is formatted (`cargo +nightly fmt --all`)
   - Clippy is happy (`cargo clippy --all-targets --all-features --locked -- -D warnings`)
   - Documentation builds (`cargo doc --no-deps --all-features --locked`)
